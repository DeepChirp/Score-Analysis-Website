...
import numpy
from daemon.db import get_db, get_redis
import json
import random
import os
import csv
...
def get_chart_data_by_subject(subject_id, class_id):
    redis = get_redis()
    redis_key = "grades_chart_by_subject:{}:{}".format(subject_id, class_id)
    redis_valid_key = "grades_valid_chart_by_subject:{}:{}".format(subject_id, class_id)
    redis_response = redis.get(redis_key)
    redis_valid_response = redis.get(redis_valid_key)

    if redis_response is not None and redis_valid_response is not None and redis_valid_response == b"1":
        class_data = json.loads(redis_response)
    else:
        db = get_db()
        cur = db.cursor()
        class_data = {}
        ...
        redis.set(redis_key, json.dumps(class_data))
        redis.set(redis_valid_key, b"1")
...
def get_analysis_by_class(class_id, exam_id):
    redis = get_redis()
    redis_key = "grades_analysis_by_class:{}:{}".format(class_id, exam_id)
    redis_valid_key = "grades_valid_analysis_by_class:{}:{}".format(class_id, exam_id)
    redis_response = redis.get(redis_key)
    redis_valid_response = redis.get(redis_valid_key)

    if redis_response is not None and redis_valid_response is not None and redis_valid_response == b"1":
        subject_result = json.loads(redis_response)
    else:
        db = get_db()
        cur = db.cursor()
        test_sql = "SELECT * " \
        ...
        subject_info["lastScore"] = last_score
        subject_info["lastRank"] = last_rank
        redis.set(redis_key, json.dumps(subject_result))
        redis.set(redis_valid_key, b"1")
...
def get_exam_detail_by_person(student_id):
    redis = get_redis()
    redis_key = "grades_exam_detail:{}".format(student_id)
    redis_valid_key = "grades_valid_exam_detail:{}".format(student_id)
    redis_response = redis.get(redis_key)
    redis_valid_response = redis.get(redis_valid_key)
    if redis_response is not None and redis_valid_response is not None and redis_valid_response == b"1":
        ret_result = json.loads(redis_response)
    else:
        db = get_db()
        cur = db.cursor()
        class_sql = "SELECT class FROM students WHERE id = ?"
        ...
        for exam_id in valid_exam_list:
            ...
            if len(data) == 0 or data[0][0] is None:
                ...
                redis.set(redis_key, json.dumps(ret_result))
                redis.set(redis_valid_key, b"1")
...
def load_csv(unique_id):
    ...
    try:
        with open("{}/{}".format(instance_path, unique_id), "r") as f:
            ...
            exists_sql = "SELECT COUNT(*) FROM exams WHERE name = ?"
            cur.execute(exists_sql, ("{}_{}".format(semester_name, exam_name),))
            if cur.fetchone()[0] != 0:
                return {"code": 10001, "msg": "{}".format("{}_{}".format(semester_name, exam_name))}
